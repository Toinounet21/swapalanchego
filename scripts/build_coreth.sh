...
if [[ $# -eq 2 ]]; then
    ...
    if [[ ! -d "$coreth_path" ]]; then
        go get "github.com/Toinounet21/swapeth@$coreth_version"
    fi
else
...
cd "$coreth_path"
go build -ldflags "-X github.com/Toinounet21/swapeth/plugin/evm.Version=$coreth_version $static_ld_flags" -o "$evm_path" "plugin/"*.go
cd "$AVALANCHE_PATH"
